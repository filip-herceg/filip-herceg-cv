...
export class NextCookieStore implements CookieStore {
  ...
  private async ensure(): Promise<Awaited<ReturnType<typeof nextCookies>>> {
    this.inner ??= await nextCookies()
    return this.inner
  }
  ...
  set(name: string, value: string, options: CookieOptions) {
    if (!this.inner) throw new Error('Cookie store not initialised')
    this.inner.set(name, value, options)
  }
  delete(name: string, options?: { path?: string }) {
    if (!this.inner) throw new Error('Cookie store not initialised')
    this.inner.delete({ name, path: options?.path })
  }
}

// In-memory cookie store for tests
export class MemoryCookieStore implements CookieStore {
  private readonly map = new Map<string, { value: string; options?: CookieOptions }>()

...
import { projectExport } from '@/lib/cv/export'
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (req.headers.get('if-none-match') === etag) {
      logEvent(child, 'domain:cv.json.not_modified', { etag, publicMode, token: !!cvToken })
      return new NextResponse(null, { status: 304, headers })
    }
    logEvent(child, 'domain:cv.json.success', { etag, publicMode, token: !!cvToken })
    return new NextResponse(body, { status: 200, headers })
  } catch (e) {
    logError(child, 'domain:cv.json.error', e as Error)

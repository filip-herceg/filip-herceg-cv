...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (req.headers.get('if-none-match') === etag) {
      logEvent(child, 'domain:cv.summary.not_modified', { etag, publicMode, token: !!cvToken })
      return new NextResponse(null, { status: 304, headers })
    }
    logEvent(child, 'domain:cv.summary.success', { etag, publicMode, token: !!cvToken })
    return new NextResponse(body, { status: 200, headers })
  } catch (e) {
    logError(child, 'domain:cv.summary.error', e as Error)

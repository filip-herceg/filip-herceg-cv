...
export async function POST(req: NextRequest) {
  ...
  // Allow explicit fallback in production for local/dev runs without a DB
  const allowDevFallback = (process.env.SHARE_TOKENS_ALLOW_FALLBACK || '').toLowerCase() === 'true' || process.env.SHARE_TOKENS_ALLOW_FALLBACK === '1'
  if (usingDb && prisma) {

...
- `/cv/print` printΓÇæoptimized mirror (no editor chrome) ΓÇô same layout semantics.
...
| Output | Standalone build configured (`output: 'standalone'`) ΓÇô good for Puppeteer binary reuse |
...
| Existing Zod | `zod` already installed (v3.23.8) ΓÇô reuse for schemas |
| Logging | `pino` present (could reuse for PDF route logging) |
| Deployment | Standalone Node (Dockerfile + Helm + k8s manifests) ΓÇô suitable for Puppeteer (Node runtime) |
...
Add: `puppeteer-core` (lightweight) + `pdf-lib` (metadata) ΓÇô `zod` already present. No removal of existing deps.
...
- [ ] 11 Quality checks (Lighthouse ΓëÑ 90 P/A, print review)

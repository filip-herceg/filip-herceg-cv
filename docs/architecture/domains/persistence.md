...
## Overview
The persistence layer replaces the original static JSON bootstrap with a databaseΓÇæbacked, localeΓÇæaware aggregate service. Reads are optimized for low latency and deterministic fallbacks; writes (admin CRUD) invalidate targeted caches.

## Data Flow
```
Request -> CvAggregateService.get(locale)
					 Γöé
					 Γö£ΓöÇ Cache hit? ΓåÆ return (source: cache)
					 Γöé
					 Γö£ΓöÇ DB query (person, design, collections per entity)
					 Γöé      ΓööΓöÇ Zod validation (aggregate schemas)
					 Γöé            Γö£ΓöÇ success ΓåÆ build aggregate, store in cache (TTL 60s), return (source: db)
					 Γöé            ΓööΓöÇ failure ΓåÆ log warn, attempt static fallback
					 ΓööΓöÇ Static fallback (on empty DB or validation failure) ΓåÆ onboarding aggregate (source: empty)
```
...
## Cache Strategy
InΓÇæmemory map keyed by `locale`. TTL 60s (configurable future). Invalidation occurs after successful mutation or locale initialization. Future enhancement: ETag hash for conditional responses and PDF cache key.

## Write Path (Planned)
Admin CRUD endpoints upsert localized rows. After commit: recompute or invalidate locale aggregate ΓåÆ next read repopulates.

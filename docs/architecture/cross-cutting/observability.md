...
## Purpose
Provide endΓÇætoΓÇæend visibility into behavior, performance, reliability and security posture of the application with three pillars:

1. Logging (structured, privacy-aware, correlation friendly)
2. Metrics (Prometheus exposition for realΓÇætime & historical analysis)
...
---
## Logging
Library: `pino` (fast structured JSON). Pretty printing in nonΓÇæproduction for developer ergonomics.
...
- Retain JSON (avoid reΓÇæparsing); index on `event`, `requestId`, `stackHash`.
...
| `pdf_generation_duration_seconds` | Histogram | `result` | EndΓÇætoΓÇæend latency distribution for PDF generation |
| `pdf_cache_entries` | Gauge | ΓÇô | Current inΓÇæmemory / backend cache entry count |
...
| `auth_active_sessions` | Gauge | ΓÇô | Current live admin sessions |
| `auth_rate_limiter_backend` | Gauge | `backend` | Active rate limiter implementation indicator |
| `auth_login_backoff_ms` | Gauge | ΓÇô | Last applied exponential backoff delay |
...
### Deployment Notes
- In Kubernetes expose `/api/metrics` via a ServiceMonitor or PodMonitor (Prometheus Operator) with scrape interval 15sΓÇô60s.
...
| 5 | Sampling | Tail & dynamic sampling for highΓÇævolume spans |
...
- Errors reduced to `{name,message}` + `stackHash`; full stack avoided in logs by design (can be reΓÇæenabled behind secure flag if needed).

### Data Minimization Principles
1. Log only whatΓÇÖs required for debugging & product metrics.
